...
class Twitter4JTwitterClientSpec extends UnitTestSpec with TweetMatcher {
	...
	describe("The client") {
		it("Should return an empty list when there are no mentions") {
			...
			BDDMockito.given(twitter.getMentionsTimeline(any(classOf[Paging]))).willReturn(responseList)
		...
		it("Should return mentions since the specified Tweet Id") {
			...
			client.fetchMentions(sinceTweetId)
			...
			Mockito.verify(twitter).getMentionsTimeline(expectedPaging)
